...
3. **Renouvellement automatique (optionnel, activé par le client)** — Après un premier paiement réussi, le client peut activer le renouvellement automatique en donnant son numéro Mobile Money et son opérateur (MTN, Moov...) — nécessaire car l'écran de paiement standard (Mobile Money/Wave/carte réunis) ne permet pas de récupérer ces informations de façon exploitable pour déclencher un prélèvement plus tard. Sans cette activation, le client reste simplement en renouvellement manuel (étape 1, chaque mois) — un mode qui fonctionne déjà très bien seul. Une tâche planifiée quotidienne (Vercel Cron, `GET /api/cron/renouvellement`) recherche les comptes Premium dont `premiumExpiresAt` est atteint ET qui ont activé le renouvellement automatique, puis déclenche un prélèvement direct Mobile Money (`sendNow`) vers le numéro enregistré. Le client reçoit une demande de confirmation sur son téléphone (code PIN), sans avoir besoin d'ouvrir MargoPro. La confirmation est traitée exactement comme à l'étape 2, par le même webhook.
...
- **Nouveau** `frontend/app/api/paiement/activer-auto/route.ts` — enregistre le numéro Mobile Money + opérateur du client (crée/relie un client FedaPay) pour activer le renouvellement automatique optionnel.
- **Modifié** `frontend/app/abonnement/page.tsx` — bouton "Renouveler" branché sur `/api/paiement/creer` + FedaPay Checkout (au lieu de la modale "Bientôt disponible"), + section "Activer le renouvellement automatique" (numéro + opérateur) affichée une fois Premium actif.
